...
void ParametersPlotting(TCanvas* c, TFitResultPtr r){ //can be improvedS
    ...
    TString s1 = Form("m = %3.3f #pm %3.3f [cm bar]",m,erm);
    TLatex* t1 = new TLatex(2,8,s1);
    t1->SetTextFont(42);
    t1->SetLineWidth(2);
    t1->Draw();
    TString s2 = Form("q = %3.3f #pm %3.3f [cm]",q,erq);
    TLatex* t2 = new TLatex(2,8.1,s2);
    t2->SetTextFont(42);
    t2->SetLineWidth(2);
    t2->Draw();
    TString s3 = Form("#rho(m,q) = %3.3f",cov_mq);
    TLatex* t3 = new TLatex(2,8.2,s3);
    t3->SetTextFont(42);
    t3->SetLineWidth(2);
    t3->Draw();
    TString s4 = Form("#chi^{2} / ndf = %3.3f / %d",chi2,r->Ndf());
    TLatex* t4 = new TLatex(2,8.4,s4);
    t4->SetTextFont(42);
    t4->SetLineWidth(2);
    t4->Draw();
    TString s5 = Form("Prob = %3.3f",prob);
    TLatex* t5 = new TLatex(2,8.6,s5);
    t5->SetTextFont(42);
    t5->SetLineWidth(2);
    t5->Draw();
...
void FitRange(const char* fname) {
    ...
    g->SetMarkerColor(4);
    g->SetMarkerStyle(20);
    g->SetTitle("Plot Range vs P^{-1} ^{244}Cm");
    ...
    gr->SetTitle("Plot Range vs P^{-1} ^{244}Cm, residui");
    ...
    grn->SetTitle("Plot Range vs P^{-1} ^{244}Cm, residui normalizzati");
